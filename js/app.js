...
重点：TOEFL Junior 侧重学术写作基础能力，强调观点清晰和逻辑组织，语言要求比 FCE 低一些`,
  ...
  // 批改标准弹窗：整合「批改风格 / 自定义评分标准 / 快速填入模板」
  function openRubricStandard() {
    const s = Storage.Settings.get() || {};
    if (els.styleSelect) els.styleSelect.value = s.style || 'standard';
    const ri = $('rubricInput');
    if (ri) ri.value = s.rubric || '';
    updateRubricState();
    els.rubricModal.hidden = false;
  }

  function closeRubricStandard() {
    els.rubricModal.hidden = true;
  }

  function saveRubricStandard() {
    const cur = Storage.Settings.get() || {};
    const settings = {
      ...cur,
      style: els.styleSelect ? els.styleSelect.value : (cur.style || 'standard'),
      rubric: ($('rubricInput') ? $('rubricInput').value : '').trim(),
    };
    Storage.Settings.save(settings);
    closeRubricStandard();
    toast('批改标准已保存', 'success');
  }
  ...
  async function startGrading(text, settings, studentName) {
    ...
    try {
      ...
      state.currentHistoryId = rec && rec.id;
  ...
  // 根据当前是否处于编辑模式，同步批注正文 / 建议句子 / 总评的 contenteditable，
  // 以及把评分数字在「数字展示 ↔ 数字输入框」之间切换（contenteditable 与输入框都无法靠 CSS 控制，必须在 JS 切换）
  function applyEditMode() {
    ...
    document.querySelectorAll('#scoreCards .score-value[data-score-key]').forEach(valEl => {
      ...
      const isTotal = key && key.toLowerCase() === 'total';
      if (state.annotationEditing) {
        if (valEl.querySelector('.score-input')) return; // 已是输入框，跳过
        if (isTotal) {
          // 总分：不转输入框，标记为自动并按各项平均重算
          const card = valEl.closest('.score-card');
          if (card) card.classList.add('score-total-auto');
          recomputeTotal();
          return;
        }
        const numEl = valEl.querySelector('.score-num');
        const raw = (numEl ? numEl.textContent : valEl.textContent).trim();
        ...
        input.max = '100';
        input.value = raw === '--' ? '' : raw;
        if (numEl) numEl.replaceWith(input);
        else { valEl.textContent = ''; valEl.appendChild(input); }
      } else {
        const card = valEl.closest('.score-card');
        if (card) card.classList.remove('score-total-auto');
        const input = valEl.querySelector('.score-input');
        if (input) {
          let v = parseFloat(input.value);
          const text = isNaN(v) ? '--' : String(Math.max(0, Math.min(100, v)));
  ...
  function syncScoresFromDOM() {
    if (!state.currentEssay) return;
    document.querySelectorAll('#scoreCards .score-input').forEach(input => {
      const key = input.dataset.scoreKey;
      if (!key) return;
      let v = parseFloat(input.value);
      if (isNaN(v)) v = 0;
      state.currentEssay.scores[key] = Math.max(0, Math.min(100, v));
    });
  }

  // 总分按各项平均分自动重算（仅当存在 total 维度时生效；总分本身不参与平均）
  function recomputeTotal() {
    if (!state.currentEssay) return null;
    const scores = state.currentEssay.scores;
    if (!scores) return null;
    const keys = Object.keys(scores);
    const totalKey = keys.find(k => k.toLowerCase() === 'total');
    if (!totalKey) return null;
    const subKeys = keys.filter(k => k !== totalKey);
    ...
    if (n === 0) return null;
    const avg = Math.round(Math.max(0, Math.min(100, sum / n)));
    scores[totalKey] = avg;
    // 编辑态下总分是只读 span（不会变成输入框），实时刷新其显示
    const valEl = document.querySelector(`#scoreCards .score-value[data-score-key="${totalKey}"]`);
    if (valEl && !valEl.querySelector('.score-input')) {
      const numEl = valEl.querySelector('.score-num');
      if (numEl) numEl.textContent = String(avg);
      else valEl.textContent = String(avg);
    }
    return avg;
  }

  // 评分卡编辑：输入框实时写回 state 并持久化（事件委托，绑定一次即可）
  function bindScoreEditing() {
    ...
    cards.addEventListener('input', (e) => {
      ...
      if (!input) return;
      const key = input.dataset.scoreKey;
      if (!key || !state.currentEssay) return;
      let v = parseFloat(input.value);
      if (isNaN(v)) v = 0;
      state.currentEssay.scores[key] = Math.max(0, Math.min(100, v));
      recomputeTotal();
    ...
    cards.addEventListener('change', (e) => {
      ...
      input.value = v;
      const key = input.dataset.scoreKey;
      if (key && state.currentEssay) state.currentEssay.scores[key] = v;
      recomputeTotal();
      persistEdits();
    });
  }
  ...
  function bindCommentEditing() {
    ...
    // 总评（评价）：编辑模式下实时写回 state 并持久化
    const overallEl = document.getElementById('overallContent');
    if (overallEl) {
      overallEl.addEventListener('input', () => {
        if (!state.annotationEditing || !state.currentEssay) return;
        state.currentEssay.overall = overallEl.innerText.replace(/\n+$/, '');
        persistEdits();
      });
    }
  }
  ...
  function bindEvents() {
    ...
    // 批改标准弹窗（主页按钮）
    if (els.rubricStandardBtn) els.rubricStandardBtn.addEventListener('click', openRubricStandard);
    document.querySelectorAll('[data-close-rubric]').forEach(el => {
      el.addEventListener('click', closeRubricStandard);
    });
    const rubricSaveBtn = $('rubricSaveBtn');
    if (rubricSaveBtn) rubricSaveBtn.addEventListener('click', saveRubricStandard);
